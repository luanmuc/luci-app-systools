...
define Package/luci-app-systools/conffiles
/etc/config/systools
endef
...
define Package/luci-app-systools/install
	...
	$(INSTALL_BIN) $(PKG_BUILD_DIR)/root/etc/uci-defaults/99-luci-systools $(1)/etc/uci-defaults/99-luci-systools
...
if [ -x /etc/uci-defaults/99-luci-systools ]; then
	/etc/uci-defaults/99-luci-systools
...
define Package/luci-app-systools/postrm
#!/bin/sh
# 卸载后脚本 - 彻底清理，不保留用户数据
# 只在彻底删除时执行（升级时不执行）
...
if [ "$1" = "remove" ] || [ "$1" = "purge" ]; then
    # 删除 UCI 配置文件
    rm -f /etc/config/systools 2>/dev/null || true
